...
is full of redundant information), often using additional command-line utilities,
or (worse) by manual copy-pasting. The processed results are then analysed in R,
...
analyses entirely from within R. It provides a set of convenient functions that
...
but if you're interested in the technical details XXX
...
populations, with Reference populations being closer to them then are outgroup
...
to the Supplementary Section 9 of Haak et al., and an [informal write-up](https://github.com/DReichLab/AdmixTools/blob/master/pdoc.pdf)
distributed with the ADMIXTOOLS software.

Probably the simples possible case to show that qpAdm works is by returning
to the question of estimating Neanderthal ancestry proportions. Let's define:

- *target* European populations to estimate ancestry proportions for
- *source* populations as Vindija Neanderthal and African (two sources of
  ancestries in all modern humans)
- *outgroup* populations Chimp, Altai Neanderthal and Denisovan (which are further
  from the true ancestral populations as the *source* populations)
