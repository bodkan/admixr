...
Both _qpWave_ and _qpAdm_ can be though of as more complex and powerful extensions of
the basic idea behind a simple $f_4$ statistic. Building upon this idea
and generalizing it (working with matrix of $f_$ statistics, instead of just one),
the _qpWave_ method  makes it possible to find the lowest number of "streams of ancestry" 
between two groups of populations that is consistent with the data. Extending the concept 
of $f_4$ statistics even further, _qpAdm_ allows to find the proportions of ancestry
from a set of ancestral populations that contributed ancestry to our sample or
...
- Note S6 of _"[Reconstructing Native American population history]"_(https://www.nature.com/articles/nature11258) by Reich et al. This paper
first introduced the theoretical background of what later became _qpWave_.

- Supplementary Information 10 of _"[Massive migration from the steppe was a source for Indo-European languages in Europe]"_(https://www.nature.com/articles/nature14317) by Haak et al., which gives a more consise
...
rank of a matrix of all possible $f_4$ statistics $f_4(\textrm{left}_1, \textrm{left}_i; \textrm{right}_1, \textrm{right}_i)$,
where $\textrm{left}_1$ and $\textrm{right}_1$ are some fixed populations from _left_ and
_right_ sets and the $i$ and $j$ indices run over all other possible choices of
populations from both sets.

As an example, let's evaluate the number of admixture waves from _right_ =
{Yoruba, Mbuti, Alta} into _left_ = {French, Sardinian, Han}. We can
do this using the `qpWave()` function, setting arguments appropriately:
...
The `qpWave()` function will return a data frame which shows the results of a series of
matrix rank tests. The `rank` column is the matrix rank tested, `df`, `chisq` and
`tail` give ..., and `dfdiff`, `chisqdiff` and `taildiff` give the same, but always
comparing the fit to the fit of a rank immediately lower.

```{r}
result
```

In this example, we see that we cannot reject the possibility that the $f_4$
matrix has a rank $r = 0$ (`tail` $p$-value = 0.684 - not rejected; rank would
be rejected if the $p$-value is low). Because Reich et al. 2012 showed that
$r + 1 \le n$, where $n$ is the number of admixture waves, this means that the data
is consistent with _left_ populations having at least $n = 1$ streams of
ancestry from the set of _right_ populations - most likely due to Neandertal
admixture present in non-Africans today.
...
with rank $r = 1$, meaning that must have been at least $n = 2$ streams of
ancestry from _right_ to _left_ populations (because $r + 1 \le n$). Because
this happened after we introduced Papuans to the _left_ set, this could indicate
a separate pulse of archaic introgression into Papuans, which would not be
surprising given what we know about the geographical patterns of archaic
admixture in non-Africans today
...
* `patterns` evaluation of different admixture patterns (to my knowledge, this
